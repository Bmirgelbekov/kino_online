...
@db
def get_film_by_id(id_) -> Post:
    post =  Post.select(Post, fn.array_agg(Genre.title).alias('genre_titles')).join(PostGenres).join(Genre).where(Post.id == id_).group_by(Post.id).get_or_none()
